...
api.post('/auth/request', (req, res) => {
  ...
  if (!isMember && store.seatsLeft() <= 0) {
    return fail(res, 403, 'circle_full', 'تم الوصول للحد الأقصى من المستخدمين حالياً');
  }
...
api.get('/state', (req, res) => {
  ...
  // المجموعة العامة + الرسائل الخاصة المتعلقة بالمستخدم فقط
  const visibleMsgs = store.messages().filter(
    (m) => !m.to || m.userId === uid || m.to === uid
  );
  res.json({
    ok: true,
    ...
    messages: visibleMsgs.map(enrichMessage),
...
api.post('/messages', (req, res) => {
  ...
  // رسالة خاصة إن وُجد to
  let to = null;
  if (req.body?.to) {
    const peer = store.members().find((u) => u && u.id === String(req.body.to));
    if (!peer) return fail(res, 404, 'not_found', 'المستخدم غير موجود');
    if (peer.id === req.user.id) return fail(res, 400, 'bad_to', 'لا يمكن مراسلة نفسك');
    to = peer.id;
  }
  ...
  const msg = store.addMessage(req.user, { text, photo: rPhoto.photo, audio: rAudio.audio, replyTo, to });
  const enriched = enrichMessage(msg);

  if (to) {
    // رسالة خاصة: أرسل للطرفين فقط عبر البث العام (الفلترة في الواجهة)
    broadcast({ type: 'message', message: enriched });
  } else {
    broadcast({ type: 'message', message: enriched });
  }
