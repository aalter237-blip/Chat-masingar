 ...
 *
 * التطبيق مصمم لتطبيق مفتوح للجميع، لذلك لا حاجة لقاعدة بيانات
...
export function addMessage(user, { text, photo, audio, replyTo, to = null }) {
  // to = null → رسالة جماعية (المجموعة العامة)
  // to = userId → رسالة خاصة
  const msg = {
    id: uid(),
    userId: user.id,
    to: to || null,
...
/** رسائل المجموعة العامة فقط */
export function groupMessages() {
  return messages().filter((m) => !m.to);
}

/** رسائل خاصة بين مستخدمين */
export function privateMessages(userA, userB) {
  return messages().filter(
    (m) =>
      m.to &&
      ((m.userId === userA && m.to === userB) || (m.userId === userB && m.to === userA))
  );
}

/** قائمة المحادثات الخاصة لمستخدم */
export function conversationsFor(userId) {
  const map = new Map();
  for (const m of messages()) {
    if (!m.to) continue;
    let peer = null;
    if (m.userId === userId) peer = m.to;
    else if (m.to === userId) peer = m.userId;
    if (!peer) continue;
    const prev = map.get(peer);
    if (!prev || m.createdAt > prev.createdAt) map.set(peer, m);
  }
  return [...map.entries()]
    .map(([peerId, last]) => ({ peerId, last }))
