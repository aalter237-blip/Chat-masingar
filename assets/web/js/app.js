...
const S = {
  ...
  statuses: [],
  online: new Set(),
  circle: { name: 'ماسنجر لايت', total: 500 },
  ...
  appPin: null,
  activeCall: null,
  chatWith: null, // null = المجموعة العامة | userId = محادثة خاصة
};

/* ---------------------------- شاشة الدخول ---------------------------- */

async function renderLogin(root) {
  let circle = { name: 'ماسنجر لايت', members: 0, total: 500, joinCodeRequired: false };
...
async function boot(root, retryCount = 0) {
  // تحسين التكيف مع لوحة المفاتيح والجوال
  try {
    ...
    // الوضع الداكن التلقائي من النظام إن لم يُحفظ تفضيل
    if (!localStorage.getItem('wa_theme') && window.matchMedia('(prefers-color-scheme: dark)').matches) {
  ...
  try {
    ...
    S.circle = st.circle || { name: 'ماسنجر لايت', total: 500 };
...
function renderChatHeader() {
  ...
  const peer = S.chatWith ? S.members.find((m) => m.id === S.chatWith) : null;
  const headName = peer ? peer.name : S.circle.name;
  const headSub = peer
    ? (S.online.has(peer.id) ? '🟢 متصل الآن' : 'آخر ظهور قريب')
    : onlineText;
  const callTarget = peer ? peer.id : 'circle';
  const callName = peer ? peer.name : S.circle.name;

  S.headerEl.replaceChildren(
    h('div', { style: 'display:flex;align-items:center;gap:8px;flex:1;min-width:0' },
      h('button', { class: 'chat-head-back', onclick: () => { S.chatWith = null; switchTab('chatlist'); }, text: '→' }),
      h('div', { class: 'wa-conv-avatar' }, avatar(headName, 38)),
      h('div', { style: 'flex:1;min-width:0' },
        h('div', { class: 'app-title', style: 'font-size:16px', text: peer ? '💬 ' + headName : '🌐 ' + headName }),
        h('div', { class: 'app-sub', id: 'chat-head-count', text: headSub }))),
    h('div', { class: 'topbar-actions' },
      h('button', { class: 'icon-btn', title: 'مكالمة مرئية', onclick: () => startCall(callTarget, callName, true) }, '📹'),
      h('button', { class: 'icon-btn', title: 'مكالمة صوتية', onclick: () => startCall(callTarget, callName, false) }, '📞'),
...
function renderFab() {
  ...
  if (S.tab === 'chatlist') {
    S.fabEl = h('button', { class: 'fab', onclick: () => openGroupChat(), title: 'المجموعة العامة' }, '💬');
...
function renderChatList(main) {
  ...
  // المجموعة العامة (رسائل بدون to)
  const groupMsgs = S.messages.filter((m) => !m.to);
  const gLast = groupMsgs.length ? groupMsgs[groupMsgs.length - 1] : null;
  const gTime = gLast ? formatChatTime(gLast.createdAt) : '';
  const gText = gLast
    ? (gLast.author.id === S.me.id ? 'أنت: ' : '') + (gLast.text || (gLast.audio ? '🎙️ رسالة صوتية' : '📷 صورة'))
    : 'ابدأوا المحادثة 👋';

  const groupConv = h('div', { class: 'wa-conversation', onclick: () => openGroupChat() },
    ...
    h('div', { class: 'wa-conv-info' },
      h('div', { class: 'wa-conv-top' },
        h('span', { class: 'wa-conv-name', text: '🌐 ' + S.circle.name + ' (عامة)' }),
        h('span', { class: 'wa-conv-time' + (S.unread ? ' unread' : ''), text: gTime })),
      h('div', { class: 'wa-conv-bottom' },
        h('span', { class: 'wa-conv-msg', text: gText }),
        S.unread ? h('span', { class: 'wa-conv-unread', text: S.unread > 99 ? '٩٩+' : String(S.unread) }) : null)));

  // محادثات خاصة مع كل عضو
  const otherMembers = S.members.filter((m) => m.id !== S.me.id);
  const memberRows = [];

  for (const m of otherMembers) {
    const online = S.online.has(m.id);
    const memberMsgs = S.messages.filter(
      (msg) => msg.to && ((msg.author.id === S.me.id && msg.to === m.id) || (msg.author.id === m.id && msg.to === S.me.id))
    );
    ...
    const lastMText = lastM
      ...
      : (online ? 'متصل الآن — اضغط للمراسلة' : `آخر ظهور ${timeAgo(m.lastSeen)}`);
    ...
    if (S.chatFilter === 'groups') continue;
    if (S.chatFilter === 'starred' && !hasStarred) continue;

    memberRows.push(h('div', { class: 'wa-conversation', onclick: () => openPrivateChat(m) },
      h('div', { class: 'wa-conv-avatar' },
        avatar(m.name, 50),
...
function renderChat(main) {
  ...
  async function send() {
    ...
    try {
      soundFx.playSend();
      const body = { text: t, photo: pendingPhoto, replyTo: reply };
      if (S.chatWith) body.to = S.chatWith;
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        reader.onload = async () => {
          ...
          try {
            soundFx.playSend();
            const body = { audio: base64Audio, duration, replyTo: S.replyTo };
            if (S.chatWith) body.to = S.chatWith;
            const r = await api('/messages', {
              method: 'POST',
              body,
  ...
  function shareLocation() {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      try {
        const body = { text: locText }; if (S.chatWith) body.to = S.chatWith; const r = await api('/messages', { method: 'POST', body });
        mergeMessage(r.message);
      } catch (err) { toast(err.message, 'error'); }
...
function visibleMessages() {
  if (!S.chatWith) {
    // المجموعة العامة فقط
    return S.messages.filter((m) => !m.to);
  }
  const peer = S.chatWith;
  return S.messages.filter(
    (m) => m.to && ((m.author.id === S.me.id && m.to === peer) || (m.author.id === peer && m.to === S.me.id))
  );
}
...
function drawChat(container) {
  ...
  if (!msgs.length) {
    const emptyText = S.chatWith
      ? 'لا رسائل خاصة بعد 👋 اكتب أول رسالة.'
      : 'لا رسائل بعد 👋 اكتب أول رسالة لبدء المحادثة.';
    container.replaceChildren(
      h('div', { class: 'chat-encrypted-badge', text: S.chatWith ? '🔒 محادثة خاصة بينكما فقط.' : '🔒 الرسائل في المجموعة العامة.' }),
      h('div', { class: 'card empty', text: emptyText })
    );
    return;
  }
  ...
  items.push(h('div', { class: 'chat-encrypted-badge', text: S.chatWith ? '🔒 محادثة خاصة — تظهر لكما فقط.' : '🔒 المجموعة العامة — يراها كل الأعضاء.' }));
    ...
    h('div', { class: 'members-wrap', style: 'padding:12px 14px' },
      h('div', { class: 'card circle-card', style: 'border-radius:10px;text-align:center;padding:18px;background:var(--bg-card);box-shadow:var(--shadow-sm);margin-bottom:12px' },
        ...
        h('p', { class: 'hint', text: seatsLeftCount > 0
          ? `التسجيل مفتوح للجميع — يظهر الأعضاء الجدد هنا تلقائياً.`
          : 'تم الوصول للحد الأقصى مؤقتاً.' })),
...
function drawMemberRows(container) {
  container.replaceChildren(...S.members.map((m) => {
    ...
    return h('div', { class: 'member-row' },
      h('span', { class: 'avatar-wrap', onclick: () => openPrivateChat(m) },
        avatar(m.name, 44),
        h('span', { class: 'presence ' + (online ? 'on' : 'off') })),
      h('div', { class: 'member-info', onclick: () => openPrivateChat(m) },
      ...
      !isMe ? h('div', { style: 'display:flex;align-items:center;gap:6px;margin-right:auto' },
        h('button', {
          class: 'icon-btn',
          title: `رسالة خاصة لـ ${m.name}`,
          onclick: (e) => { e.stopPropagation(); openPrivateChat(m); }
        }, '💬'),
        h('button', {
          class: 'icon-btn',
...
async function startCall(targetId, targetName, isVideo = false) {
  ...
  try {
    if (!window.isSecureContext && location.protocol !== 'https:') {
      toast('المكالمات تحتاج رابط آمن (HTTPS). الرابط الحالي HTTP — المتصفح يمنع المايك والكاميرا.', 'error');
      return;
    }
    const constraints = {
      audio: true,
    ...
    if (!window.isSecureContext) {
      toast('المتصفح يمنع المايك/الكاميرا على HTTP. تحتاج HTTPS للمكالمات.', 'error');
    } else {
...
async function acceptIncomingCall(ev) {
  ...
  try {
    if (!window.isSecureContext && location.protocol !== 'https:') {
      toast('المكالمات تحتاج HTTPS. الرابط الحالي غير آمن.', 'error');
