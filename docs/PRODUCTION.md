...
|---|---|---|
| المصادقة (OTP) | `SMS_PROVIDER=whatsapp` — رمز دخول يصل على واتساب عبر Meta Cloud API الرسمية | بدون QR، بدون بوابة محلية؛ يحتاج قالب OTP معتمد (خطوات في `README.md`) |
| (بديل) SMS حقيقي | `SMS_PROVIDER=textbee` من شريحة هاتفك الخاص | إن كانت رسائل SMS لا تصل، استخدم WhatsApp أعلاه |
| (مكتب صغير) console | `SMS_PROVIDER=console` — الكود في سجل السيرفر | الأبسط؛ فقط للفريق الصغير جدًا |
